...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView }) => {
  ...
  const isStoryViewActive = activeView !== View.COMMUNITY;
  ...
  return (
    <aside className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col p-4 shadow-lg">
      ...
      {/* Story Management - Conditionally rendered */}
      <div className={isStoryViewActive ? 'opacity-100' : 'opacity-50 pointer-events-none'}>
          <div className="mb-4">
            ...
            <div className="flex items-center gap-2">
                ...
                >
                  {stories.length === 0 && <option>{t('no_stories_yet')}</option>}
                  {stories.map(story => (
      ...
      <nav className="flex-1 space-y-2">
        <NavItem icon={<PencilIcon className="h-5 w-5" />} label={t('story_editor')} view={View.STORY_EDITOR} activeView={activeView} onClick={setActiveView} disabled={!activeStoryId} />
        <NavItem icon={<UsersIcon className="h-5 w-5" />} label={t('characters')} view={View.CHARACTERS} activeView={activeView} onClick={setActiveView} disabled={!activeStoryId} />
        <NavItem icon={<GlobeIcon className="h-5 w-5" />} label={t('worlds')} view={View.WORLDS} activeView={activeView} onClick={setActiveView} disabled={!activeStoryId} />
        <NavItem icon={<ListIcon className="h-5 w-5" />} label={t('timeline')} view={View.TIMELINE} activeView={activeView} onClick={setActiveView} disabled={!activeStoryId} />
        <NavItem icon={<BibleIcon className="h-5 w-5" />} label={t('story_bible')} view={View.STORY_BIBLE} activeView={activeView} onClick={setActiveView} disabled={!activeStoryId} />
        <div className="pt-2 border-t border-gray-200 dark:border-gray-700 my-2"></div>
        <NavItem icon={<CommunityIcon className="h-5 w-5" />} label={t('community')} view={View.COMMUNITY} activeView={activeView} onClick={setActiveView} />
      </nav>
...
const NavItem: React.FC<NavItemProps> = ({ icon, label, view, activeView, onClick, disabled = false }) => {
  ...
  return (
    <a
      href="#"
      onClick={(e) => { e.preventDefault(); if(!disabled) onClick(view); }}
